...
import { 
  Search, TrendingUp, Calendar, Eye, Heart, Bookmark, Plus, 
  Code, Palette, User, Filter, Clock, Star, Zap
} from "lucide-react";
...
const personalizedTrendsData = [
  ...
  {
    id: 5,
    title: "비슷한 사용자가 좋아함: Supabase 풀스택 개발",
...
export function TrendView({ onNavigate }: TrendViewProps) {
  ...
  const getCurrentData = () => {
    switch (activeTab) {
      case "tech":
        return techTrendsData;
      case "design":
        return designTrendsData;
      case "personalized":
        return personalizedTrendsData;
      default:
        return techTrendsData;
    }
  };
  ...
  const getFilters = () => {
    switch (activeTab) {
      case "tech":
        return ["전체", "Frontend", "Backend", "AI/ML", "Language", "Infrastructure", "Performance"];
      case "design":
        return ["전체", "Tools", "UI Trend", "Design System", "Motion", "3D UI", "Accessibility"];
      case "personalized":
        return ["전체", "For You", "Recommended", "Based on Activity", "Interest Match", "Similar Users", "Skill Growth"];
      default:
        return ["전체"];
    }
  };
